...
| 51 | Executive Dashboard — project links open in new tab | ✅ Done | The "Top 10 Projects" table links to `/projects?search=<code>` now open in a new browser tab (`target="_blank" rel="noopener noreferrer"`) so the Executive Dashboard stays open. UI-only change — no new unit tests (CODEBASE_RULES §9a). |
| 52 | Projects — "Allocations" tab (Active/Upcoming/Ended) | ✅ Done | New tab on the project detail panel alongside Overview/Tasks, showing every allocation for the project as Resource Name, Start Date, End Date, Allocation %, Hours to Date, and Total Hours. Split into three sections: **Active** (today within range), **Upcoming** (start date in future), **Ended** (end date in past, collapsible/collapsed by default). Allocation % = hoursPerDay ÷ (capacity÷5) × 100; Hours to Date / Total Hours reuse the same leave-adjusted per-allocation figures as the existing Engineer Breakdown table. `getCachedAllAllocationsForProjects` now also returns `id` + `userCapacity`. New `src/lib/projectAllocationUtils.ts` (`computeAllocationPct`, `categorizeAllocation`, `groupAllocationsByCategory`) — projectAllocationUtils.test.ts (20 tests). |
