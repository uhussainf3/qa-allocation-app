...
export const getCachedActiveUsers = unstable_cache(
  async () =>
    prisma.user.findMany({
      ...
      select: { id: true, name: true, email: true, image: true, capacity: true, role: true, jobTitle: true, isOnshore: true, divisionId: true, managerId: true, department: true },
      orderBy: { name: "asc" },
    }),
  ["active-users"],
...
export const getCachedSimpleUsers = unstable_cache(
  async () =>
    prisma.user.findMany({
      ...
      select: { id: true, name: true, email: true, capacity: true, role: true, jobTitle: true, isOnshore: true, divisionId: true, managerId: true, department: true },
      orderBy: { name: "asc" },
    }),
  ["simple-users"],
  { revalidate: TTL, tags: ["users"] }
  ...
  async () => {
    const rows = await prisma.user.findMany({
      select: {
        ...
        department: true, isActive: true, isOnshore: true, divisionId: true, managerId: true, createdAt: true,
